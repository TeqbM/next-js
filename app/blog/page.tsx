      ...
      <div className="container mx-auto">
          ...
          <ul className="grid sm:grid-cols-3 grid-cols-1 gap-3">
            {data?.slice(0,cou).map((item: Posts) => (
              <li className="space-y-3 p-5 cursor-pointer border border-slate-200 hover:border-amber-600 bg-white rounded-2xl rounded-bl-none" key={item.id}>
          ...
          <div className="text-center mt-7">
            {data?.length >= cou &&
              <button className="!text-base" onClick={()=> setCou(cou+12)}>LoadMore...</button>
            }
          </div>
